# benchmarks/bench_gemm_square.py
...
def main():
    parser = argparse.ArgumentParser(description="Square GEMM benchmark: Triton vs cuBLAS (PyTorch matmul).")
    ...
    for dtype_key in args.dtypes:
        ...
        for s in args.sizes:
            ...
            for t in range(args.trials):
                ...
                ms_torch.append(mt)
                ms_triton.append(mr)
                print(f"trial {t:02d} | {s} fp={dtype_key} | torch {mt:7.3f} ms | triton {mr:7.3f} ms")
    ...
    csv_path = out_dir / f"gemm_square_bench_{gpu_name.replace(' ','_')}_{timestamp}.csv"
    ...
    plt.title(f"Square GEMM Throughput vs Size (p50) on {gpu_name}")
    plt.grid(True, linestyle="--", linewidth=0.5)
    plt.legend()
    plt.tight_layout()
    out_png = plot_dir / "gemm_square_tflops_p50.png"
    plt.savefig(out_png)
    plt.show()
    print("Saved:", out_png)

    # Plot: Speedup torch/triton vs FLOPs
    plt.figure(figsize=(12, 6))
    for dtype_key in args.dtypes:
        ...
        merged = cublas.merge(triton, on=["gpu", "op", "dtype", "M", "N", "K", "flops"], suffixes=("_c", "_t"))
    ...
    plt.title(f"Triton Speedup vs Torch (p50) on {gpu_name}")
    plt.grid(True, linestyle="--", linewidth=0.5)
    plt.legend()
    plt.tight_layout()
    out_png = plot_dir / "gemm_square_speedup_p50.png"
    plt.savefig(out_png)
    plt.show()
    print("Saved:", out_png)
